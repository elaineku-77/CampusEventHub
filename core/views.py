...
def home(request):
    ...
    highlighted_events = _prepare_events(
        published_events
    ...
    popular_events = sorted(
        _prepare_events(published_events),
        key=lambda event: event.registered_count,
        reverse=True
    )[:3]

    latest_events = _prepare_events(
        published_events.order_by("-created_at")[:3]
    )
...
def event_list(request):
    ...
    events_list = _prepare_events(events_qs)

    if filter_type == "popular":
        events_list = sorted(events_list, key=lambda e: e.registered_count, reverse=True)

    elif filter_type == "latest":
        events_list = sorted(events_list, key=lambda e: e.created_at, reverse=True)

    elif filter_type == "full_closed":
        events_list = [
            event for event in events_list
            if event.availability_class in ["full", "closed"]
        ]

    if sort_by == "seats":
        events_list = sorted(events_list, key=lambda e: e.seats_left_count, reverse=True)
...
def admin_dashboard_view(request):
    ...
    closed_events = all_events.filter(status="Closed").count()
    total_registered = Registration.objects.count()

    total_capacity = sum(event.max_participants for event in all_events)
    seats_left = sum(event.seats_left() for event in all_events)
    full_events_count = sum(1 for event in all_events if event.is_full())

    registration_rate = 0
    if total_capacity > 0:
        registration_rate = round((total_registered / total_capacity) * 100, 1)

    need_attention = sum(
        1 for event in all_events
        if event.is_full() or event.status == "Closed" or event.seats_left() <= 5
    )

    popular_events = sorted(
        all_events,
        key=lambda event: event.registration_count(),
        reverse=True
    )[:5]

    fully_registered_events = [
        event for event in all_events if event.is_full()
    ]

    latest_events = all_events[:5]

    most_popular_event = popular_events[0] if popular_events else None

    avg_registration = 0
    if total_events > 0:
        avg_registration = round(total_registered / total_events)
    ...
    category_counts = {
        "Workshop": Event.objects.filter(category="Workshop").count(),
        "Seminar": Event.objects.filter(category="Seminar").count(),
        "Social": Event.objects.filter(category="Social").count(),
        "Sports": Event.objects.filter(category="Sports").count(),
        "Technology": Event.objects.filter(category="Technology").count(),
        "Arts": Event.objects.filter(category="Arts").count(),
    }
    ...
    return render(request, "admin_dashboard.html", {
        ...
        "full_events": full_events_count,
        "registration_rate": registration_rate,
        "need_attention": need_attention,

        "popular_events": popular_events,
        "fully_registered_events": fully_registered_events,
        "latest_events": latest_events,
        "most_popular_event": most_popular_event,
        "avg_registration": avg_registration,

        "workshop_count": Event.objects.filter(category="Workshop").count(),
        "seminar_count": Event.objects.filter(category="Seminar").count(),
        "social_count": Event.objects.filter(category="Social").count(),
        "sports_count": Event.objects.filter(category="Sports").count(),
        "technology_count": Event.objects.filter(category="Technology").count(),
        "arts_count": Event.objects.filter(category="Arts").count(),
...
def admin_event_detail_view(request, event_id):
    ...
    return render(request, 'admin_event_detail.html', {
        'event': event,
        'registrations': registrations,
        'registration_count': event.registered_count,
        'seats_left': event.seats_left_count,
    })
